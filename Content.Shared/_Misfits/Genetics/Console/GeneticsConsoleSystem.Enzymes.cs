...
public sealed partial class GeneticsConsoleSystem
{
    ...
    private void OnSaveEnzymes(Entity<GeneticsConsoleEnzymesComponent> ent, ref GeneticsConsoleSaveEnzymesMessage args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Low, $"{args.Actor:user} saved {name:name}'s unique enzymes to {disk.Owner:disk} with console {ent.Owner:console}");
    ...
    private void OnApplyEnzymesDoAfter(Entity<GeneticsConsoleEnzymesComponent> ent, ref ApplyEnzymesDoAfterEvent args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.High, $"{args.User:user} applied unique enzymes of '{enzymes.Name:name}' to {mob:target} with console {ent.Owner:console}");
